...
for i=1:1:(p-1)  % 100 predictions, therefore 100 iterations
    input = [TrainData(p,(end-p+1+i):end) predictions];  % the input of the network must contain 
    pred = sim(net1,input');  % simulate the networks with the input vector
    predictions = [predictions pred];
end

disp(["MSE= " mse(predictions,norm_test_data) "  " hidden " neurons"])

rmse = sqrt(mean((predictions-norm_test_data).^2));
